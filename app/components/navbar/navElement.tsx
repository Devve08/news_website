...
const NavElement = ({ title, dropdown }: NavElementProps) => {
  ...
  return (
    <div onClick={dropdown?.length > 0 ? handleDropdown: ()=>{}} className=" relative flex flex-row items-center justify-start gap-1 cursor-pointer hover:opacity-90 transition-opacity duration-300">
      ...
      <AnimatePresence>
        {dropdown.length > 0 && dropdownOpen && (
          ...
          >
            <div className=" text-white font-medium font-sans text-base hover:opacity-80 transition-opacity p-2 ">
              {title}
            </div>
          </motion.div>
        )}
